...
class DataLoader:
    ...
    def load_day(self, instrument: str, date_str: str, 
                 ...
                 time_range: Optional[Tuple[str, str]] = None) -> pl.DataFrame:
        """
        ...
        Returns:
            Polars DataFrame with tick data, sorted by timestamp
        """
        ...
        try:
            ...
            if record_types:
                df = df.filter(pl.col('record_type').is_in(record_types))
    ...
    def load_range(self, instrument: str, start_date: str, end_date: str,
                   record_types: Optional[List[str]] = None,
                   chunk_size: Optional[int] = None) -> Union[pl.DataFrame, Generator[pl.DataFrame, None, None]]:
        """
        ...
        Returns:
            Polars DataFrame with combined tick data from all dates
        """
        ...
        for date_str in date_range:
            ...
            if chunk_size and len(dataframes) >= chunk_size:
                ...
                chunk_df = pl.concat(dataframes)
                chunk_df = chunk_df.sort('timestamp')
    ...
    def load_time_window(self, instrument: str, date_str: str, 
                        ...
                        record_types: Optional[List[str]] = None) -> pl.DataFrame:
        """
    ...
    def get_date_summary(self, instrument: str, date_str: str) -> Dict:
        ...
        l1_count = df.filter(pl.col('record_type') == 'L1').height
        l2_count = df.filter(pl.col('record_type') == 'L2').height
    ...
    def validate_data_integrity(self, instrument: str, date_str: str) -> Dict:
        ...
        l1_pct = (df.filter(pl.col('record_type') == 'L1').height / len(df)) * 100
        l2_pct = (df.filter(pl.col('record_type') == 'L2').height / len(df)) * 100
