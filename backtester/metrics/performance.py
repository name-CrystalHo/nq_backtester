"""
Performance Metrics - Generate metrics matching NinjaTrader's Strat        # Basic trade statistics
        ...
        self.gross_profit = stats.get('gross_profit', 0.0)
        self.gross_loss = stats.get('gross_loss', 0.0)
        self.profit_factor = stats.get('profit_factor', 0.0)
        self.avg_win = stats['avg_win']
        self.avg_loss = stats['avg_loss']
        self.largest_win = stats.get('largest_win', 0.0)
        self.largest_loss = stats.get('largest_loss', 0.0)
...
class PerformanceMetrics:
    ...
    def export_ninja_format(self, filename: str):
        ...
        df = pd.DataFrame(list(ninja_metrics.items()), columns=['Metric', 'Value'])
